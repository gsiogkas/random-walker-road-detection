function [mask,probabilities] = random_walker(img,seeds,labels,conn,thr,beta)
...
if(Z == 3) %Color images
    ...
    tmp=img(:,:,3);
    imgVals(:,3)=tmp(:);
elseif(Z > 3 | Z == 2) %Multimodal Images
    imgVals = zeros(X*Y,Z);
    tmp = zeros(X,Y);
    for zind = 1:Z
        tmp = img(:,:,zind);
...
% Weight thresholding (Causes detail enhancement)
if thr > 0
